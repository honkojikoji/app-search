...
async function scrapeAndWriteCSV() {
  ...
  // CSVファイルにデータを書き込む
  const csvData = allData.map(row => `${row.id},${row.word_class},${row.root_or_mood},${row.gender_or_person},${row.case_or_voice},${row.number},${row.ending_pattern},${row.url}`).join('\n');
  fs.writeFileSync('../../pali.csv', csvData);
  ...
  db.serialize(() => {
    db.run(`CREATE TABLE IF NOT EXISTS pali (
    ...
    const insertStmt = db.prepare(`INSERT INTO pali (word_class, root_or_mood, gender_or_person, case_or_voice, number, ending_pattern, url)
      VALUES (?, ?, ?, ?, ?, ?, ?)`);
...
function modifyData(data, tdCount, h2Text, url, column, wordClass ,index, tableIndex) {
  ...
  if (!h2Text.includes('性') && tdCount < 9) {
    if(data[lastIndex].root_or_mood == 'manas'){
      data[lastIndex].gender_or_person = "中性";
    }else if(data[lastIndex].root_or_mood.includes('at')){
      if(tableIndex == 1){
        data[lastIndex].gender_or_person = "女性";
      }else{
        if(column < 4){
          data[lastIndex].gender_or_person = "男性";
        }else{
          data[lastIndex].gender_or_person = "中性";
        }
      }
    }else{
      data[lastIndex].gender_or_person = "性なし";
    }
  }
  if (url.includes('tb=na&bm=aia')) {
    if (column > 7) {
      data[lastIndex].root_or_mood = 'ī'
    } else {
      data[lastIndex].root_or_mood = 'a'
    }
  }
  if (url.includes('tb=pr&bm=so')) {
    if (column < 4) {
      data[lastIndex].root_or_mood = 'so'
    } else if (column < 7) {
      data[lastIndex].root_or_mood = 'taṃ'
    } else {
      data[lastIndex].root_or_mood = 'sā'
    }
  }
  if (data[lastIndex].root_or_mood == 'atvatmat') {
    data[lastIndex].root_or_mood = '(vm)at'
  }
  if (url.includes('tb=num') && url.includes('bm=c05')) {
    data[lastIndex].number = '複';
  }
  ...
  if (wordClass == "数詞") {
    ...
    if (tdCount == 2) {
      data[lastIndex].gender_or_person = '男中女性';
    } else if (tdCount == 4) {
      if (column == 2) {
        data[lastIndex].gender_or_person = '男性';
      } else if (column == 3) {
    ...
    if(url.includes('bm=c05')){
      if(index==0){
        ...
        data[lastIndex].root_or_mood = 'dasa';
      }
    }
  }
}
