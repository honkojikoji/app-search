...
const db = new sqlite3.Database('pali.db');
const tableName = "pali";
const replacements = {
  'A': 'ā',
  'I': 'ī',
  'U': 'ū',
...
router.get('/', (req, res, next) => {
  ...
  res.render('pali', data);
});

/* search検索用 */
router.get('/search', (req, res) => {
  let keyword = replaceTextWithMap(req.query.pali_search, replacements);
  let flg_space;
  if (keyword == "") {
    var data = {
      title: "Pali 検索",
    ...
    db.serialize(() => {
      db.all(query, array, (err, rows) => {
        if (!err) {
          ...
          res.render('pali', data); // EJSテンプレートをレンダリング
        }
      });
    });
  }
