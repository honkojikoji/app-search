...
router.get('/search', (req, res) => {
  let keyword = replaceTextWithMap(req.query.test_search, replacements);
  let flg_space;
  if (keyword == "") {
    var data = {
      title: "Sanskrit 検索",
    ...
    const query = `SELECT * FROM ${tableName} WHERE ending_pattern IN (${placeholders}) OR REPLACE(ending_pattern, 'a', 'ā') IN (${placeholders})`;
    db.serialize(() => {
      db.all(query, [...array, ...array], (err, rows) => {
        if (!err) {
          var data = {
            title: keyword,
            content: rows,
          ...
          res.render('test', data); // EJSテンプレートをレンダリング
        }
      });
    });
  }
